    ...
    for sub_task in sub_tasks:
        if 'day_0' in sub_task['criteria']:
            for whitelist in Whitelist.objects.all():
                ...
                if currentDay != 0:
                    continue
                print(whitelist.uuid, sub_task["id"])
            ...
            for user in WebUser.objects.all():
                ...
                if 'survey_not_done' in sub_task['criteria']:
                    ...
                    if anchor is None or user.currentDay < anchor or user.currentDay > anchor:
                        continue
                ...
                elif 'banned' in sub_task["criteria"] and not user.banNotified:
                    if not any(x in sub_task["criteria"] for x in banTags):
                        continue
                    WebUser.objects.filter(uuid=user.uuid).update(banNotified=True)
                    banLog = True
                    
                elif 'all' in sub_task["criteria"]:
                    pass
                
                else: 
                    continue
                
                print(user.uuid, sub_task["id"])
